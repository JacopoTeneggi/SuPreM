...
def process(args):
    ...
    if (
        ...
        model = networkarch.SMIT_3D_Seg(
            config, out_channels=args.num_class, pretrain=args.pretrain
        )
    ...
    if args.model_backbone == "swinunetr":
        ...
        print(
            f"Use {args.pretraining_method_name} SwinUnetr backbone pretrained weights"
        )
    ...
    if args.model_backbone == "unet":
        ...
        if args.pretrain is not None:
            ...
            print(
                f"Use {args.pretraining_method_name} UNet backbone pretrained weights"
            )
        else:
    ...
    if args.model_backbone == "segresnet":
        ...
        if args.pretrain is not None:
            ...
            print(amount, len(store_dict.keys()))
            print(
                f"Use {args.pretraining_method_name} SegResNet backbone pretrained"
                " weights"
            )
        else:
    ...
    while args.epoch < args.max_epoch:
        ...
        if args.epoch % args.store_num == 0:
            ...
            if mean_dice > best_dice:
                ...
                torch.save(
                    checkpoint,
                    os.path.join("checkpoints", args.log_name, "best_model.pth"),
                )
...
def main():
    ...
    parser.add_argument(
        "--max_epoch", default=201, type=int, help="Number of training epoches"
    )
    parser.add_argument(
