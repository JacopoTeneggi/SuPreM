...
def get_default_configuration(
    ...
    assert network in [
        ...
        "network can only be one of the following: '3d', '3d_lowres', '3d_fullres',"
        " '3d_cascade_fullres'"
    )
    ...
    if (network == "3d_cascade_fullres" or network == "3d_lowres") and len(
        ...
        raise RuntimeError(
            "3d_lowres/3d_cascade_fullres only applies if there is more than one stage."
            " This task does not require the cascade. Run 3d_fullres instead"
        )
    ...
    output_folder_name = join(
        network_training_output_dir, network + "_" + plans_identifier, task, outname
    )
