...
def drop_path(x, drop_prob: float = 0.0, training: bool = False):
    ...
    shape = (x.shape[0],) + (1,) * (
        x.ndim - 1
    )  # work with diff dim tensors, not just 3D ConvNets
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        self, img_size=[16, 96, 96], patch_size=[16, 16, 16], in_chans=1, embed_dim=768
    ):
        super().__init__()
        num_patches = (
...
class MiT(nn.Module):
    def __init__(
        ...
        # Encoder
        if encoder == "small":
            self.transformer = MiT_encoder.model_small(
                norm_cfg=norm_cfg,
            ...
            self.transformer = MiT_encoder.model_tiny(
                norm_cfg=norm_cfg,
                activation_cfg=activation_cfg,
                weight_std=weight_std,
                img_size=img_size,
        ...
        self.DecEmbed0 = PatchEmbed(
            img_size=[img_size[0] // 8, img_size[1] // 16, img_size[2] // 16],
            patch_size=[2, 2, 2],
            in_chans=self.transformer.embed_dims[-1],
            embed_dim=embed_dims[0],
        )
        self.DecEmbed1 = PatchEmbed(
            img_size=[img_size[0] // 4, img_size[1] // 8, img_size[2] // 8],
            patch_size=[2, 2, 2],
            in_chans=embed_dims[0],
            embed_dim=embed_dims[1],
        )
        self.DecEmbed2 = PatchEmbed(
            img_size=[img_size[0] // 2, img_size[1] // 4, img_size[2] // 4],
            patch_size=[2, 2, 2],
        ...
        # Decoder patchEmbed
        self.DecPosEmbed0 = nn.Parameter(
            torch.zeros(1, self.transformer.patch_embed3.num_patches, embed_dims[0])
        )
        self.DecPosDrop0 = nn.Dropout(p=drop_rate)
        self.DecPosEmbed1 = nn.Parameter(
            torch.zeros(1, self.transformer.patch_embed2.num_patches, embed_dims[1])
        )
        self.DecPosDrop1 = nn.Dropout(p=drop_rate)
        self.DecPosEmbed2 = nn.Parameter(
            torch.zeros(1, self.transformer.patch_embed1.num_patches, embed_dims[2])
        )
        ...
        self.transposeconv_stage3 = nn.ConvTranspose3d(
            embed_dims[2], embed_dims[3], kernel_size=2, stride=2
        )
        self.stage3_de = Conv3dBlock(
            embed_dims[3],
            embed_dims[3],
            norm_cfg=norm_cfg,
            activation_cfg=activation_cfg,
        ...
        # Seg head
        self.ds0_cls_conv = nn.Conv3d(
            embed_dims[0], self.MODEL_NUM_CLASSES, kernel_size=1
        )
        self.ds1_cls_conv = nn.Conv3d(
            embed_dims[1], self.MODEL_NUM_CLASSES, kernel_size=1
        )
        self.ds2_cls_conv = nn.Conv3d(
            embed_dims[2], self.MODEL_NUM_CLASSES, kernel_size=1
        )
        self.ds3_cls_conv = nn.Conv3d(
            embed_dims[3], self.MODEL_NUM_CLASSES, kernel_size=1
        )

        self.cls_conv = nn.Conv3d(embed_dims[3], self.MODEL_NUM_CLASSES, kernel_size=1)

        trunc_normal_(self.DecPosEmbed0, std=0.02)
        trunc_normal_(self.DecPosEmbed1, std=0.02)
        trunc_normal_(self.DecPosEmbed2, std=0.02)
    ...
    def forward(self, inputs):
        ...
        x_trans = (
            x_encoder[-1].reshape(B, D, H, W, -1).permute(0, 4, 1, 2, 3).contiguous()
        )
    ...
    def __init__(
        ...
        if pretrain:
            ...
            if pre_type == "teacher":
                ...
                pre_dict_ori = {
                    k.replace("backbone.", ""): v for k, v in pre_dict_ori.items()
                }
                print(
                    "Teacher: length of pre-trained layers: %.f" % (len(pre_dict_ori))
                )
            else:
                pre_dict_ori = torch.load(pretrain_path, map_location="cpu")[pre_type]
                pre_dict_ori = {
                ...
                print(
                    "Student: length of pre-trained layers: %.f" % (len(pre_dict_ori))
                )
            ...
            print(
                "before loading weights: %.12f"
                % (
                    self.model.state_dict()[
                        "transformer.block1.0.mlp.fc1.weight"
                    ].mean()
                )
            )
            ...
            # Patch_embeddings
            print(
                "Patch_embeddings layer1 weights: %.12f"
                % (
                    self.model.state_dict()[
                        "transformer.patch_embed1.proj.conv.weight"
                    ].mean()
                )
            )
            print(
                "Patch_embeddings layer2 weights: %.12f"
                % (
                    self.model.state_dict()[
                        "transformer.patch_embed2.proj.conv.weight"
                    ].mean()
                )
            )
            ...
            # Position_embeddings
            print(
                "Position_embeddings weights: %.12f"
                % (self.model.transformer.pos_embed1.data.mean())
            )

            if model_dict["transformer.patch_embed0.conv.weight"].shape[1] != 1:
                pre_dict_ori["transformer.patch_embed0.conv.weight"] = pre_dict_ori[
                    "transformer.patch_embed0.conv.weight"
                ].repeat_interleave(
                    model_dict["transformer.patch_embed0.conv.weight"].shape[1], 1
                )
                ...
                if "2D" not in k:
                    if ("transformer.pos_embed" in k) or ("DecPosEmbed" in k):
                        ...
                        if posemb.size() == posemb_new.size():
                            ...
                            posemb_zoom = ndimage.zoom(
                                posemb[0], (ntok_new / posemb.size(1), 1), order=1
                            )
            ...
            print(
                "Patch_embeddings layer1 pretrained weights: %.12f"
                % (
                    self.model.state_dict()[
                        "transformer.patch_embed1.proj.conv.weight"
                    ].mean()
            ...
            print(
                "Patch_embeddings layer2 pretrained weights: %.12f"
                % (
                    self.model.state_dict()[
                        "transformer.patch_embed2.proj.conv.weight"
                    ].mean()
            ...
            print(
                "Position_embeddings pretrained weights: %.12f"
                % (self.model.transformer.pos_embed1.data.mean())
            )
