...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(
        ...
        assert step_size <= 1, (
            "step_size must be smaller than 1. Otherwise there will be a gap between"
        ...
        if len(mirror_axes):
            ...
            if (
                ...
                or self.conv_op
                == diynnu.network_architecture.ResTranUnet_ssl_pure.Conv3d_wd
            ):
        ...
        if self.training:
            print(
                "WARNING! Network is in train mode during inference. This may be"
                " intended, or not..."
            )
        ...
        with context():
            with torch.no_grad():
                if (
                    ...
                    raise RuntimeError(
                        "Invalid conv op, cannot determine what dimensionality (2d/3d)"
                        " the network is"
                    )
    ...
    def predict_2D(
        ...
        assert step_size <= 1, (
            "step_size must be smaler than 1. Otherwise there will be a gap between"
            " consecutive predictions"
        )
