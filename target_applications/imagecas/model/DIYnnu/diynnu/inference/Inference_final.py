...
def process_nii(file_path, save_path):
    ...
    nib.save(
        nib.Nifti1Image(reordered_img, affine=new_nii.affine, header=new_header),
        save_path,
    )
...
def processing(sample_name, img):
    os.makedirs(
        os.path.join(preprocessed_data_path, "image", sample_name), exist_ok=True
    )
    shutil.copy(
        img,
        os.path.join(
            preprocessed_data_path, "image", sample_name, sample_name + "_0000.nii.gz"
        ),
    )
    process_nii(
        os.path.join(
...
def deprocessing(sample_name):
    ...
    seg_total, spacing, origin, direction = NiiDataRead(
        os.path.join(output_path, sample_name, sample_name + ".nii.gz")
    )
    ...
    for itr, class_name in enumerate(eval_class):
        ...
        NiiDataWrite(
            os.path.join(
                output_path, sample_name, "predictions", class_name + ".nii.gz"
            ),
            tmp_seg,
            spacing,
            origin,
...
for data_name in tqdm(all_data_name):
    ...
    os.system(
        f"CUDA_VISIBLE_DEVICES={gpu_id} python predict_simple.py -i"
        f' {os.path.join(preprocessed_data_path, "image", sample_name)}             -mp'
        f" checkpoints/ -o {os.path.join(output_path, sample_name)} -p nnUNetPlansv2.1"
        " -t Task066_AbdomenAtlas1.0 -m 3d_fullres  -tr nnUNetTrainerV2_ResTrans -f"
        " all -chk model_best"
    )
