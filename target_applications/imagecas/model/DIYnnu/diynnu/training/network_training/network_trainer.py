...
class NetworkTrainer(object):
    ...
    def load_best_checkpoint(self, train=True):
        ...
        if isfile(join(self.output_folder, "model_best.model")):
            ...
            self.print_to_log_file(
                "WARNING! model_best.model does not exist! Cannot load best checkpoint."
                " Falling back to load_latest_checkpoint"
            )
    ...
    def run_training_dis(self):
        ...
        if cudnn.benchmark and cudnn.deterministic:
            warn(
                "torch.backends.cudnn.deterministic is True indicating a deterministic"
                " training is desired. But torch.backends.cudnn.benchmark is True as"
                " well and this will prevent deterministic training! If you want"
                " deterministic then set benchmark=False"
            )
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        ...
                        tbar.set_postfix(
                            loss=l, loss_su=l_su, loss_un=l_un, moment_t=moment_t
                        )
            ...
            self.print_to_log_file(
                "train un loss : %.4f" % np.mean(train_losses_epoch_un)
            )
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l, l_su, l_un, moment_t = self.run_iteration(
