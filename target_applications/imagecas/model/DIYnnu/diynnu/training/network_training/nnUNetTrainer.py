...
class nnUNetTrainer(NetworkTrainer):
    ...
    def update_fold(self, fold):
        ...
        if fold is not None:
            if isinstance(fold, str):
                assert (
                    fold == "all" or "xf"
                ), "if self.fold is a string then it must be 'all'"
    ...
    def plot_network_architecture(self):
        try:
            ...
            if torch.cuda.is_available():
                g = hl.build_graph(
                    self.network,
                    torch.rand((2, self.num_input_channels, *self.patch_size)).cuda(),
                    transforms=None,
                )
            else:
                g = hl.build_graph(
    ...
    def process_plans(self, plans):
        if self.stage is None:
            assert len(list(plans["plans_per_stage"].keys())) == 1, (
                "If self.stage is None then there can be only one stage in the plans"
                " file. That seems to not be the case. Please specify which stage of"
                " the cascade must be trained"
            )
        ...
        if "pool_op_kernel_sizes" not in stage_plans.keys():
            ...
            self.print_to_log_file(
                "WARNING! old plans file with missing pool_op_kernel_sizes. Attempting"
                " to fix it..."
            )
            self.net_num_pool_op_kernel_sizes = []
        ...
        if "conv_kernel_sizes" not in stage_plans.keys():
            self.print_to_log_file(
                "WARNING! old plans file with missing conv_kernel_sizes. Attempting to"
                " fix it..."
            )
        ...
        if (
            ...
            print(
                "WARNING! You seem to have data that was preprocessed with a previous"
                " version of nnU-Net. You should rerun preprocessing. We will proceed"
                " and assume that both transpose_foward and transpose_backward are [0,"
                " 1, 2]. If that is not correct then weird things will happen!"
            )
    ...
    def finish_online_evaluation(self):
        ...
        self.print_to_log_file(
            "(interpret this as an estimate for the Dice of the different classes. This"
            " is not exact.)"
        )
