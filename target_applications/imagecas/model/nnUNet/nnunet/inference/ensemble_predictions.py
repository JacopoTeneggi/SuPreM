    ...
    if override or not isfile(out_file):
        ...
        if not all([i is None for i in reg_class_orders]):
            ...
            for r in reg_class_orders[1:]:
                assert tmp == r, (
                    "If merging files with regions_class_order, the"
                    " regions_class_orders of all files must be the same."
                    " regions_class_order: %s, \n files: %s"
...
def merge(
    ...
    for f in folders:
        assert all(
            [isfile(join(f, i + ".npz")) for i in patient_ids]
        ), "Not all patient npz are available in all folders"
        assert all(
            [isfile(join(f, i + ".pkl")) for i in patient_ids]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "This script will merge predictions (that were prdicted with the "
    ...
    parser.add_argument(
        ...
        "--postprocessing_file",
        help=(
            "path to the file where the postprocessing configuration "
