...
def main():
    ...
    parser = argparse.ArgumentParser(
        usage=(
            ...
            " cross-validation. Running this script requires all models to have been"
            " run already. This script will summarize the results of the five folds of"
            " all models in one json each for easy interpretability"
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "set this flag if you want this script to crash of one of the models is"
            " missing"
        ),
    )
    ...
    for t in tasks:
        ...
        for m in models:
            try:
                ...
                output_folder = get_output_folder_name(
                    m, id_task_mapping[t], trainer, pl
                )
                assert isdir(
                    output_folder
        ...
        for k, v in results.items():
            if v == best:
                ...
                best_model = k
                print(
                    "\nHere is how you should predict test cases. Run in sequential"
                    " order and replace all input and output folder names with your"
                    " personalized ones\n"
                )
                if k.startswith("ensemble"):
                    ...
                    predict_str += (
                        "nnUNet_predict -i FOLDER_WITH_TEST_CASES -o"
                        " OUTPUT_FOLDER_MODEL1 -tr "
                        + tr
                        + " -ctr "
                        + trc
                        + " -m "
                    ...
                    predict_str += (
                        "nnUNet_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2"
                        " -o OUTPUT_FOLDER -pp "
                        + join(
                            network_training_output_dir,
                            "ensembles",
