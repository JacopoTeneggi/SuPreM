...
def ensemble(
    ...
    for f in folds:
        ...
        patient_identifiers1 = subfiles(
            validation_folder_net1, False, None, "npz", True
        )
        patient_identifiers2 = subfiles(
            validation_folder_net2, False, None, "npz", True
        )
        ...
        assert all(
            [isfile(join(validation_folder_net1, i)) for i in all_patient_identifiers]
        )
        assert all(
            [isfile(join(validation_folder_net2, i)) for i in all_patient_identifiers]
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        usage=(
            "This is intended to ensemble training images (from cross-validation) only."
            " Useinference/ensemble_predictions.py instead"
        )
    )
    ...
    parser.add_argument(
        "--folds", nargs="+", type=int, default=(0, 1, 2, 3, 4), required=False
    )
