...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "List of integers belonging to the task ids you wish to run"
    ...
    parser.add_argument(
        ...
        help=(
            "Name of the ExperimentPlanner class for the full resolution 3D U-Net and"
            " U-Net cascade. Default is ExperimentPlanner3D_v21. Can be 'None', in"
            " which case these U-Nets will not be configured"
        ),
    )
    parser.add_argument(
        "-pl2d",
        ...
        help=(
            "Name of the ExperimentPlanner class for the 2D U-Net. Default is"
            " ExperimentPlanner2D_v21. Can be 'None', in which case this U-Net will not"
            " be configured"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Set this flag if you dont want to run the preprocessing. If this is set"
            " then this script will only run the experiment planning and create the"
            " plans file"
        ),
    )
    parser.add_argument(
        ...
        default=4,
        help=(
            "Number of processes used for preprocessing the low resolution data for the"
            " 3D low resolution U-Net. This can be larger than -tf. Don't overdo it or"
    ...
    parser.add_argument(
        ...
        default=4,
        help=(
            "Number of processes used for preprocessing the full resolution data of the"
            " 2D U-Net and 3D U-Net. Don't overdo it or you will run out of RAM"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "set this flag to check the dataset integrity. This is useful and should be"
            " done once for each dataset!"
        ),
    )
    ...
    if planner_name3d is not None:
        ...
        if planner_3d is None:
            raise RuntimeError(
                "Could not find the Planner class %s. Make sure it is located somewhere"
                " in nnunet.experiment_planning" % planner_name3d
            )
    else:
        planner_3d = None
    ...
    if planner_name2d is not None:
        ...
        if planner_2d is None:
            raise RuntimeError(
                "Could not find the Planner class %s. Make sure it is located somewhere"
                " in nnunet.experiment_planning" % planner_name2d
            )
    else:
        planner_2d = None
