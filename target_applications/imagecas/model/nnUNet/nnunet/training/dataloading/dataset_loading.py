...
def crop_2D_image_force_fg(img, crop_size, valid_voxels):
    ...
    if type(crop_size) not in (tuple, list):
        ...
        assert len(crop_size) == (len(img.shape) - 1), (
            "If you provide a list/tuple as center crop make sure it has the same len"
            " as your data has dims (3d)"
        )
        ...
        for j, i in enumerate(selected_keys):
            ...
            if self.has_prev_stage:
                ...
                assert all(
                    ...
                    "seg_from_previous_stage does not match the shape of case_all_data:"
                    " %s vs %s"
                    % (
            ...
            if not force_fg:
                ...
                if "class_locations" not in properties.keys():
                    raise RuntimeError(
                        "Please rerun the preprocessing with the newest version of"
                        " nnU-Net!"
                    )
