        ...
        if not self.was_initialized:
            ...
            if training:
                if not isdir(self.folder_with_segs_from_prev_stage):
                    raise RuntimeError(
                        "Cannot run final stage of cascade. Run corresponding 3d_lowres"
                        " first and predict the segmentations for the next stage"
                    )
    ...
    def validate(
        ...
        if do_mirroring:
            if not self.data_aug_params["do_mirror"]:
                raise RuntimeError(
                    "We did not train with mirroring so you cannot do inference with"
                    " mirroring enabled"
                )
