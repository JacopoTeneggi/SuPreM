...
from nnunet.dataset_conversion.Task056_Verse_normalize_orientation import (
    normalize_slice_orientation,
    read_image,
    restore_original_slice_orientation,
    save_image,
)
...
if __name__ == "__main__":
    ...
    train_files_base = subfiles(
        join(base_corrOrient, "train"), join=True, suffix="_seg.nii.gz"
    )
    train_segs = [i[: -len("_seg.nii.gz")] + "_seg.nii.gz" for i in train_files_base]
    train_data = [i[: -len("_seg.nii.gz")] + ".nii.gz" for i in train_files_base]
    test_files_base = [
        i[: -len(".nii.gz")]
        for i in subfiles(join(base_corrOrient, "test"), join=True, suffix=".nii.gz")
    ]
    test_data = [i + ".nii.gz" for i in test_files_base]
    ...
    train_patient_names = [
        i[: -len("_seg.nii.gz")]
        for i in subfiles(
            join(base_corrOrient, "train"), join=False, suffix="_seg.nii.gz"
        )
    ]
    ...
    test_patient_names = [
        i[:-7]
        for i in subfiles(join(base_corrOrient, "test"), join=False, suffix=".nii.gz")
    ]
    ...
    folder_gt = (
        "/media/fabian/My"
        " Book/MedicalDecathlon/nnUNet_raw_splitted/Task056_VerSe/labelsTr"
    )
