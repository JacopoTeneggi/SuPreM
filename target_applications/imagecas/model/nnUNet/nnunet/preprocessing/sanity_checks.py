...
def verify_dataset_integrity(folder):
    ...
    for c in expected_train_identifiers:
        ...
        for i, img in enumerate(images_itk):
            ...
            if not same_geometry:
                geometries_OK = False
                print(
                    "The geometry of the image %s does not match the geometry of the"
                    " label file. The pixel arrays will not be aligned and nnU-Net"
                    " cannot use this data. Please make sure your image modalities are"
                    " coregistered and have the same geometry as the label"
    ...
    for i, r in enumerate(results):
        if not r[0]:
            print(
                "Unexpected labels found in file %s. Found these unexpected values"
                " (they should not be there) %s" % (label_files[i], r[1])
            )
            fail = True

    if fail:
        raise AssertionError(
            "Found unexpected labels in the training dataset. Please correct that or"
            " adjust your dataset.json accordingly"
        )
    else:
        print("Labels OK")
    ...
    if len(expected_test_identifiers) > 0:
        ...
        assert len(nii_files_in_imagesTs) == 0, (
            "there are training cases in imagesTs that are not listed in"
            " dataset.json: %s" % nii_files_in_imagesTr
        )
    ...
    if not all_same:
        print(
            "WARNING: Not all images in the dataset have the same axis ordering. We"
            " very strongly recommend you correct that by reorienting the data."
            " fslreorient2std should do the trick"
        )
    # save unique orientations to dataset.json
    if not geometries_OK:
        raise Warning(
            "GEOMETRY MISMATCH FOUND! CHECK THE TEXT OUTPUT! This does not cause an"
            " error at this point  but you should definitely check whether your"
            " geometries are alright!"
        )
    else:
        print("Dataset OK")
    ...
    if has_nan:
        raise RuntimeError(
            "Some images have nan values in them. This will break the training. See"
            " text output above to see which ones"
        )
