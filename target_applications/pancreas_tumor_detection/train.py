...
def process(args):
        ...
        if args.pretrain is not None:
            ...
            for key in store_dict.keys():
                if key in simplified_model_dict and "conv_final.2.conv" not in key:
                    ...
                    amount += 1
            assert amount == (
                len(store_dict.keys()) - 2
            ), "the pre-trained model is not loaded successfully"
    ...
    if rank == 0:
        writer = SummaryWriter(
            log_dir=os.path.join(args.log_checkpoint_savepath, args.log_name)
        )
        print(
            "Writing Tensorboard logs to ",
            os.path.join(args.log_checkpoint_savepath, args.log_name),
        )
    ...
    while args.epoch <= args.max_epoch:
        ...
        if args.epoch % args.store_num == 0:
            ...
            torch.save(
                checkpoint,
                os.path.join(
                    args.log_checkpoint_savepath,
                    args.log_name,
                    "checkpoint_epoch_" + str(args.epoch) + ".pth",
                ),
            )
        ...
        torch.save(
            checkpoint,
            os.path.join(args.log_checkpoint_savepath, args.log_name, "model.pth"),
        )
...
def main():
    ...
    ## logging
    parser.add_argument(
        "--log_name", default="unet_organ", help="The path resume from checkpoint"
    )
    parser.add_argument(
        "--log_checkpoint_savepath",
        default="out",
    ...
    parser.add_argument(
        "--max_epoch", default=200, type=int, help="Number of training epoches"
    )
    parser.add_argument(
        "--store_num", default=50, type=int, help="Store model how often"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=16,
        type=int,
        help="Number of initial filters in segresnet",
    )
    ...
    parser.add_argument(
        "--a_min", default=-100, type=float, help="a_min in ScaleIntensityRanged"
    )
    parser.add_argument(
        "--a_max", default=200, type=float, help="a_max in ScaleIntensityRanged"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--map_type",
        default="jhh",
        help="organs | muscles | cardiac | vertebrae | ribs",
    )
    parser.add_argument(
        "--num_class", default=5, type=int, help="class num: 18 | 22 | 19 | 25 | 25"
    )
    ...
    parser.add_argument(
        "--model_backbone",
        default="unet",
        help="model backbone, also avaliable for swinunetr",
    )
    parser.add_argument(
        "--print_params",
        action="store_true",
        default=False,
        help="print model parameter numbers",
    )
