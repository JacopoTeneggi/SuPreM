...
def split_file(args):
    ...
    for line in lines:
        ...
        if line_count == args.lines_per_file:
            ...
            subset_number += 1
            subset_file_path = (
                f"{file_name}_{args.file_suffix}_{subset_number}{args.extension}"
            )
            subset_file_path = os.path.join(args.save_dir, subset_file_path)
            subset_file = open(subset_file_path, "w")
            line_count = 0
...
def main():
    ...
    parser.add_argument(
        "--save_dir", required=True, help="Directory to save the split files"
    )
    parser.add_argument(
        "--lines_per_file", type=int, default=30, help="Number of lines per file"
    )
    parser.add_argument(
        "--file_suffix", default="subset", help="Suffix for the split files"
    )
    parser.add_argument(
        "--extension", default=".txt", help="Extension for the split files"
    )
