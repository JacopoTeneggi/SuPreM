...
def validation(model, ValLoader, val_transforms, args):
    ...
    for index, batch in enumerate(tqdm(ValLoader)):
        ...
        align_pred = np.where(
            np.isin(pred, [2, 3, 4]), pred + 1, pred
        )  # change labels for pred: 1,2,3,4 -> 1,3,4,5
        nib.save(
            nib.Nifti1Image(align_pred.astype(np.uint8), original_affine),
            file_path_pattern,
        )

        # save probabilities for each class
        if args.saveprobabilities:
            probabilities_save_path = os.path.join(
                save_dir, name[0].split("/")[0], "probabilities"
            )
            ...
            for k in range(1, args.num_class):
                ...
                pred_class_prob = (pred_class_prob * 255).astype(
                    np.uint8
                )  # convert to 0-255
                file_path_pattern = os.path.join(
                    probabilities_save_path, f"{class_name}.nii.gz"
                )
                nib.save(
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="save the probabilities of the model",
    )
