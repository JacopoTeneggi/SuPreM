...
def get_loader(args):
    ...
    data_dicts_test = [
        {"image": image, "label": label, "post_label": post_label, "name": name}
        for image, label, post_label, name in zip(
            test_img, test_lbl, test_post_lbl, test_name
        )
    ]
...
def get_loader_without_gt(args):
    ...
    for item in args.dataset_list:
        for line in open(args.data_txt_path + item + "_test.txt"):
            ...
            test_name.append(name)
    data_dicts_test = [
        {"image": image, "name": name} for image, name in zip(test_img, test_name)
    ]
    print("test len {}".format(len(data_dicts_test)))

    if args.cache_dataset:
        test_dataset = CacheDataset(
            data=data_dicts_test, transform=val_transforms, cache_rate=args.cache_rate
        )
    else:
        test_dataset = Dataset(data=data_dicts_test, transform=val_transforms)
    test_loader = DataLoader(
