...
def validation(model, ValLoader, args):
    ...
    if dice_results:
        with open(
            os.path.join(
                output_directory,
                f"{args.train_type}_{args.model_backbone}_{args.map_type}_dice_validation_results.csv",
            ),
            "w",
            newline="",
        ) as file:
    ...
    if nsd_results:
        with open(
            os.path.join(
                output_directory,
                f"{args.train_type}_{args.model_backbone}_{args.map_type}_nsd_validation_results.csv",
            ),
            "w",
            newline="",
        ) as file:
...
def main():
    ...
    parser.add_argument(
        "--trans_encoding",
        default="rand_embedding",
        help="the type of encoding: rand_embedding or word_embedding",
    )
    parser.add_argument(
        "--word_embedding",
        default="./pretrained_weights/word_embedding.pth",
    ...
    parser.add_argument(
        "--a_min", default=-250, type=float, help="a_min in ScaleIntensityRanged"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--map_type", default="vertebrae", help="sample number in each ct"
    )
    ...
    parser.add_argument(
        "--train_type", default="scratch", help="either train from scratch or transfer"
    )
    parser.add_argument(
        "--percent", default=1081, type=int, help="pre-training using numbers of images"
    )
