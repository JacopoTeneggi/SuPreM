...
def h5_to_nifti(h5_path, nifti_path):
    with h5py.File(h5_path, "r") as h5f:
        ...
        header_dict = {
            key: header_group.attrs[key] for key in header_group.attrs.keys()
        }
...
def main(debug_mode=False):
    ...
    for CT_ID in tqdm(CT_ID_LIST):
        ...
        REVISED_MASK_FILE = [
            s.replace(ORIGINAL_ROOT_DIR, REVISED_ROOT_DIR) for s in ORIGINAL_MASK_FILE
        ]
        ...
        if debug_mode:
            ...
            h5_to_nifti(
                h5_path=REVISED_CT_FILE,
                nifti_path=ORIGINAL_CT_FILE.replace(
                    ORIGINAL_ROOT_DIR, REREVISED_ROOT_DIR
                ),
            )

            if not os.path.exists(
                os.path.join(REREVISED_ROOT_DIR, CT_ID, "segmentations")
            ):
                os.makedirs(os.path.join(REREVISED_ROOT_DIR, CT_ID, "segmentations"))
            REREVISED_MASK_FILE = [
                s.replace(ORIGINAL_ROOT_DIR, REREVISED_ROOT_DIR)
                for s in ORIGINAL_MASK_FILE
            ]
            for revised_mask, rerevised_mask in zip(
                REVISED_MASK_FILE, REREVISED_MASK_FILE
            ):
