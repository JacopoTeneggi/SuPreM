...
def main(args):
    ...
    folder_names = [
        name
        for name in os.listdir(args.datapath)
        if os.path.isdir(os.path.join(args.datapath, name))
    ]
    ...
    for pid in folder_names:
        ...
        if error_detected:
            ...
            with open(
                os.path.join(args.csvpath, args.csvname), "a", newline=""
            ) as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([pid])

    print(
        "\n> Overall error report {:.1f}% = {}/{}".format(
            100.0 * len(error_list) / len(folder_names),
            len(error_list),
            len(folder_names),
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="csvpath",
        type=str,
        default="/Users/zongwei.zhou/Desktop/error_analysis",
