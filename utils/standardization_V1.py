...
def main():
    ...
    parser.add_argument(
        "--ROOT_DIR", default=None, type=str, help="original root directory"
    )
    parser.add_argument(
        "--ROOT_UNIQUE_DIR", default=None, type=str, help="revised root directory"
    )
    ...
    for ct_id in tqdm(CT_ID_LIST):
        if os.path.exists(os.path.join(args.ROOT_DIR, ct_id, "segmentations")):
            img = nib.load(
                os.path.join(args.ROOT_DIR, ct_id, "segmentations/liver.nii.gz")
            )
        else:
        ...
        for i in range(len(dic)):
            if data.shape == dic[i]["dimension"]:
                ...
                ct2 = nib.load(
                    os.path.join(
                        args.ROOT_UNIQUE_DIR, rename_id(dic[i]["ID"]), "ct.nii.gz"
                    )
                )
                ...
                if abs(np.sum(ct1 > 0) - np.sum(ct2 > 0)) < 20:
                    print(
                        "{} = {} = {}".format(
                            ct_id, dic[i]["ID"], abs(np.sum(ct1 > 0) - np.sum(ct2 > 0))
                        )
                    )
                    duplication = True
                    break
        ...
        if duplication:
            ...
            dic.append(case)
            shutil.move(
                os.path.join(args.ROOT_DIR, ct_id),
                os.path.join(args.ROOT_UNIQUE_DIR, rename_id(ct_id)),
            )
