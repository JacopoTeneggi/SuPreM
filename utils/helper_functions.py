...
def aorta_error(pid, datapath):
    ...
    if (liver.shape[0] == 512 and liver.shape[1] == 512) or (
        ...
        for i in range(liver.shape[-1]):
            if error2d_isliver_noaorta(
                liver[:, :, i], aorta[:, :, i]
            ) or error2d_ispostcava_islung_nonaorta(
                postcava[:, :, i],
                lung_right[:, :, i],
                lung_left[:, :, i],
                aorta[:, :, i],
...
def kidney_postprocessing(pid, datapath):
    kidney_left, kidney_left_affine, kidney_left_header = load_mask(
        pid, "kidney_left", datapath
    )
    kidney_right, kidney_right_affine, kidney_right_header = load_mask(
...
def error2d_ispostcava_islung_nonaorta(postcava, lung_right, lung_left, aorta):
    ...
    if (
        np.sum(postcava) > 0
        and (np.sum(lung_right) > 0 or np.sum(lung_left) > 0)
        and np.sum(aorta) == 0
    ):
        return True
    else:
